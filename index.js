...
var mongodb, MongoClient, _, async, request, fs, idkfile, e, db, collection, config, defaultQueueSettings, jobValidation, Job, Queue, updateConfig, createJob, args, queues, express, app, bodyParser, router, toString$ = {}.toString;
...
Job = (function(){
  ...
  prototype.checkBatchFinish = function(next){
    ...
    collection.find({
      ...
      if (count === 0) {
        db.collection('batch').findAndModify({
...
Queue = (function(){
  ...
  prototype.process = function(job){
    var that;
    if (that = idkfile[job.option('type')]) {
      return that(job);
    }
...
queues = {};
fs.watch('./idkfile.js', function(){
  var e;
  delete require.cache[require.resolve('./idkfile.js')];
  idkfile = (function(){
    try {
      return require('./idkfile.js');
    } catch (e$) {
      e = e$;
      return {};
    }
