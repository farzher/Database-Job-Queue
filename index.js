...
var mongodb, MongoClient, _, async, request, fs, Path, Url, db, collection, dbConfig, queues, defaultQueueConfig, jobValidation, Job, Queue, reloadDbConfig, createJob, promoteJobs, configFile, exists, configFileAbsPath, configObject, e, toString$ = {}.toString, slice$ = [].slice;
...
Queue = (function(){
  ...
  prototype.processPendingJobs = function(){
    ...
    (function(next){
      ...
      var resetAt;
      resetAt = Date.now() + (this$.options.duration || queues['default'].duration) * 1000;
...
MongoClient.connect(configObject.connect, function(err, _db){
  ...
  collection.createIndexes([
    ...
    router.all('/job/update', function(req, res){
      collection.find({
        ...
        if (!job) {
          ...
          args = req.body.call;
          job[args[0]].apply(job, slice$.call(args, 1));
    ...
    router.all('/job/poll', function(req, res){
      ...
      resetAt = Date.now() + ((queue != null ? (ref$ = queue.options) != null ? ref$.duration : void 8 : void 8) || queues['default'].duration) * 1000;
      collection.findOneAndUpdate({
        type: req.body.type,
        state: 'pending'
      }, {
    ...
    router.all('/get-db-config', function(req, res){
      res.send(dbConfig);
    });
    router.all('/save-db-config', function(req, res){
      delete req.body._id;
      db.collection('config').find().limit(1).next(function(err, _dbConfig){
